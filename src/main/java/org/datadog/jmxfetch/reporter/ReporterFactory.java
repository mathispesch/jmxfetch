    ...
    public static Reporter getReporter(AppConfig appConfig) {
        ...
        if ("console".equals(type)) {
            ...
            if (matcher.find() && matcher.groupCount() == 2) {
                ...
                return new StatsdReporter(host, port, appConfig.getStatsdTelemetry(), appConfig.getStatsdQueueSize());
            }

            matcher = Pattern.compile("^statsd:unix://(.*)$").matcher(type);
            if (matcher.find() && matcher.groupCount() == 1) {
                String socketPath = matcher.group(1);
                return new StatsdReporter(socketPath, 0, appConfig.getStatsdTelemetry(), appConfig.getStatsdQueueSize());
            }
        }
